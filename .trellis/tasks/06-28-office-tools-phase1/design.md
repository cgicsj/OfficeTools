...
- Main process: native file/folder dialogs, output path handling, Downloads directory resolution, cache directory management, WPS invocation, and job orchestration.
...
- local path access;
- WPS detection and conversion calls;
...
- `WorkbookAdapter`: open converted `.xlsx`, inspect workbook/sheet metadata, read display values, copy style-sensitive ranges, and write output `.xlsx`.
- `WpsConverter`: detect usable WPS command support and convert `.xls` / `.et` to temporary `.xlsx`.
...
4. Main process converts where needed, opens workbook metadata, and reports sheets/row counts/field-name candidates.
...
4. Main process converts where needed, validates selected sheets, merges outputs, resolves filename conflicts, writes `汇总数据.xlsx` or a renamed variant, and reports completion.

## Compatibility Notes

- UOS ARM64 and WPS command behavior are high-risk and must be tested on target hardware.
- Electron packaging must not bundle or assume WPS; it should locate and invoke the system installation.
- The app should degrade clearly when WPS is unavailable.
- `.xls` and `.et` fidelity depends on WPS conversion. Any conversion loss is outside OfficeTools' direct control but should be logged.
...
- WPS-only conversion simplifies Phase 1 but ties `.xls` and `.et` support to installed system software.
