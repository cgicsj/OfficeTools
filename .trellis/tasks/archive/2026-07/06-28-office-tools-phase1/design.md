...
- Main process: native file/folder dialogs, output path handling, Downloads directory resolution, cache directory management, direct `.xls` / `.et` reader orchestration, and job orchestration.
...
- local path access;
- direct `.xls` / `.et` library read calls;
...
- `DirectWorkbookReader`: open legacy `.xls` and WPS `.et` files directly through the selected library without WPS conversion and expose the workbook metadata needed by split/merge.
...
The initial probe result selects `exceljs` as the first `.xlsx` workbook adapter candidate, SheetJS CE `xlsx` as the direct `.xls` / `.et` reader candidate, and `yauzl` relationship inspection for selected-sheet embedded-object detection. Feature implementation may build behind these adapter boundaries, but direct `.xls` and `.et` reading remain gated by representative samples. Formula output should use cached calculated results; files without saved formula results need an explicit warning/skip path.
...
- Use SheetJS CE `xlsx` as the direct `.xls` / `.et` reader candidate; do not convert `.xls` or `.et` through WPS.
...
4. Main process opens `.xls` and `.et` through the direct reader where needed, opens workbook metadata, and reports sheets/row counts/field-name candidates.
...
4. Main process opens `.xls` and `.et` through the direct reader where needed, validates selected sheets, merges outputs, resolves filename conflicts, writes `汇总数据.xlsx` or a renamed variant, and reports completion.

## Compatibility Notes

- `.xls` and `.et` fidelity depends on the direct reader's metadata support and must be validated with real samples.
- Direct `.xls` parsing needs codepage support for non-ASCII legacy workbooks.
...
- Direct `.xls` / `.et` reading removes dependence on installed WPS, but it creates metadata fidelity gates for both legacy and WPS-authored formats.
